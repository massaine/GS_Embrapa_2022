---
title: "7_Predict_Parental_BV_Dom"
author: "massaine"
date: "2022-07-01"
...
## Nao foi analisado (Erro na funcao runGenomicPredictions())
...
grms<-list(D=readRDS(file=here::here("output","GDommatrixGBSandDArTGS.rds")))
...
blups %<>% dplyr::select(Trait,fitAS) %>% unnest(fitAS) %>% dplyr::select(Trait,blups) %>% 
     ...
     dplyr::mutate(TrainingData=map(TrainingData,
                             ~filter(.,germplasmName %in% rownames(grms$D)) %>% 
...
dim(D)
grms[["D"]]<-D
blups$TrainingData[[1]]$GID

gpreds_withMarkEffs.D<-runGenomicPredictions(modelType = "AD", 
...
## Save the results
```{r}
saveRDS(gpreds_withMarkEffs.D,file = here::here("output","genomicPredictions_withMarkEffs.rds"))
```
